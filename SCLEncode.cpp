...
#define LAB  str = ""; while(blankSpace(l[pos])) { pos++; }  while (validChar(l[pos])) { str.push_back(l[pos]); pos++; } add = curSubr[subIndex].getLabAdd(str); if(curSubr[subIndex].localLab.find(str) != curSubr[subIndex].localLab.end()){fwrite(&add, 4, 1, out); outPos += 4;}else{printf("Error on line %d: Local label not found.\n"); exit(5);}
						...
						case AJMP:
							...
							add = curSubr[subIndex].getLabAdd(str); 
							if (curSubr[subIndex].localLab.find(str) != curSubr[subIndex].localLab.end()) 
							{ 
								fwrite(&add, 4, 1, out); 
								outPos += 4;
							}
							else { printf("Error on line %d: Local label not found.\n", ln); exit(5); }
